  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // ✅ Payment Data for Dainik Bhaskar
    const payments = [
      { month: "जनवरी", amount: 1500, status: "Paid", date: "05-01-2025" },
      { month: "फरवरी", amount: 1300, status: "Paid", date: "07-02-2025" },
      { month: "मार्च", amount: 1000, status: "Pending", date: "-" },
    ];

    let totalPaid = payments.filter(p => p.status === "Paid")
                            .reduce((sum, p) => sum + p.amount, 0);
    let totalDue = payments.filter(p => p.status !== "Paid")
                           .reduce((sum, p) => sum + p.amount, 0);
    ...
    if (sidebar) {
      ...
      toggleBtn.addEventListener("click", () => sidebar.classList.toggle("sidebar-show"));
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // 🧠 Translate function (partial + flexible)
  function translateText(text, lang) {
    const dict = lang === "hi" ? translations : reverseTranslations;
    for (const [key, val] of Object.entries(dict)) {
      // make sure small pieces like "Founded 1958" also translate even with dots/spaces
  ...
  // 🔄 Translate visible text nodes safely
